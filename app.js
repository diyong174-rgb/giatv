...
const channels = {
  ...
  { name: 'Dateline', url: 'http://161.49.17.2:6610/001/2/ch00000090990000001093/manifest.mpd?JITPDRMType=Widevine&virtualDomain=001.live_hls.zte.com', type: 'mpd', keyId: '002029fcdaae2385eedf0fd9332c3c90', key: '33b97956f316d0b65e9fd28888e7c7a9', logo: '' }
	 ...
	 MOVIES: [
		 ...
		 { name: 'LION KING', url: 'https://fonts.gstatic127.workers.dev/NOTFLIX_PH?file=0m3FseIWxPd3you8cROo7T3ZMC3f1oato2EyltnGJyUZR2TqoQjb0OIxG4Erc25r&expiry=qdwg7gmcJ%2FcdMC7tBrgJ7Q%3D%3D&mac=4e0f440bec5178a95d18ba29bf0dbd46a249a7e41a72c36c4192b05e0c88b046', type: 'movies', logo: '' }
		  ],
      RADIO: [
...
function destroyPlayers() {
  ...
  if (hlsInstance) {
    try {
      hlsInstance.destroy();  // Destroy HLS.js player
    } catch (e) {
  ...
  // Destroy DASH (Shaka Player) instance
  if (shakaPlayerInstance) {
    try {
      shakaPlayerInstance.destroy();  // Destroy Shaka Player
    } catch (e) {
  ...
  // Destroy other possible players like MP4 or MP3 players
  if (dashInstance) {
    try {
      dashInstance.reset();  // Reset DASH player instance
    } catch (e) {
      console.error("Error destroying DASH instance:", e);
    }
    dashInstance = null;
  }
}
...
function playChannel(channel) {
  ...
  destroyPlayers();  // Ensure previous players are destroyed
  container.innerHTML = '';  // Clear the container before adding a new player
  ...
  if (channel.type === 'mpd') {
    ...
    // DRM configuration for MPD streams
  ...
  if (type === 'hls' || (/\.m3u8($|\?)/i).test(channel.url)) {
    ...
    if (Hls.isSupported()) {
      const hls = new Hls();
      hls.loadSource(channel.url); // Load HLS stream URL
