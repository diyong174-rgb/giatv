...
function trackActiveViewers() {
  let viewers = localStorage.getItem('viewers') || 0;
  viewers = parseInt(viewers) + 1;
  localStorage.setItem('viewers', viewers);

  // Update viewer count text
  document.getElementById('viewerCountText').innerText = `${viewers} viewers`;
}

window.onload = () => {
  trackActiveViewers(); // Increment viewers on page load
};
...
const channels = {
 CABLE: [
  ...
  { name: 'SINEMANILA', url: 'https://live20.bozztv.com/giatv/giatv-sinemanila/sinemanila/chunks.m3u8', type: 'hls', logo: '' },
   { name: 'GMA 7', url: 'https://vod.nathcreqtives.com/1093/manifest.mpd', keyId: '31363231383438333031323033393138', key: '38694e34324d543478316b7455753437', type: 'mpd', logo: '' },	 	   
  ...
  { name: 'Dateline', url: 'http://161.49.17.2:6610/001/2/ch00000090990000001093/manifest.mpd?JITPDRMType=Widevine&virtualDomain=001.live_hls.zte.com', type: 'mpd', keyId: '002029fcdaae2385eedf0fd9332c3c90', key: '33b97956f316d0b65e9fd28888e7c7a9', logo: '' }
      ...
      YOUTUBE: [
         { name: 'YOUTUBE LIVE MOVIES', url: 'https://www.youtube.com/embed/-SojqU6N5Wk', type: 'youtube', logo: '' },
		{ name: 'YOUTUBE LIVE MOVIES 2', url: 'https://www.youtube.com/embed/e7GPAy9vq04', type: 'youtube', logo: '' },
	 ...
	 MOVIES: [
		 ...
		 { name: 'Ziam (tagalog)', url: 'https://urosrv1.streambucket.xyz/hls2/01/00138/fle7t48m3xnv_x/index-v1-a1.m3u8?t=sKhPjSIbHjARBms5JuH5H3bQGBmLE0Roc8jpYiy4iBI&s=1759029460&e=7200&v=66701455&srv=stora1&i=0.3&sp=2000', type: 'movies', logo: '' }
		  ],
      RADIO: [
...
function playChannel(channel) {
  ...
  if (channel.type === 'hls') {
  ...
  } else if (channel.type === 'movies') {
    videoPlayer.src = channel.url;
    videoPlayer.style.display = 'block';
    videoPlayer.play();
  } else if (channel.type === 'mpd') {
    currentPlayer = new shaka.Player(videoPlayer);
    ...
    currentPlayer.load(channel.url).then(() => {
      ...
      console.error('Error loading MPD stream:', error);
    });
  } else if (channel.type === 'mp3') {
...
function searchChannels() {
  const searchInput = document.getElementById('searchChannel').value.toLowerCase();
  const allChannels = document.querySelectorAll('.channel-list div');
...
window.onload = () => {
  loadCategory('CABLE');
  playChannel(channels.CABLE[0]);
  makeScrollable(); // Make the drawer and list scrollable
};

// Add this function to handle scrolling for categories and channels
function makeScrollable() {
  const categoryDrawer = document.getElementById('categoryDrawer');
  const channelList = document.getElementById('channelList');
  
  // Enable scrollable behavior for both
  categoryDrawer.style.overflowX = 'auto'; // Horizontal scroll for category drawer
  channelList.style.overflowY = 'auto'; // Vertical scroll for channel list
  channelList.style.maxHeight = '300px'; // Limit height for scrolling channels
}
