...
// Set username display after login
...
function showApp() {
  ...
  showChannelList('CABLE'); // Display channels immediately after login
}
...
const channels = {
  ...
  { name: 'GMA 7', url: 'https://tinyurl.com/GMA-Kapuso', type: 'hls', logo: '' },  
  { name: 'GTV', url: 'https://tinyurl.com/GTv-PH-Channel', type: 'hls', logo: '' },   	   
  ...
  { name: 'Dateline', url: 'http://161.49.17.2:6610/001/2/ch00000090990000001093/manifest.mpd?JITPDRMType=Widevine&virtualDomain=001.live_hls.zte.com', type: 'mpd', keyId: '002029fcdaae2385eedf0fd9332c3c90', key: '33b97956f316d0b65e9fd28888e7c7a9', logo: '' }
      ...
      YOUTUBE: [
         { name: 'Mr Bean LIVE! Classic Comedy', url: 'https://www.youtube.com/embed/amzgpxDsJjQ', type: 'youtube', logo: '' },
		{ name: 'LIVE! 24 HOURS Pixar Cars NEW Livestream!', url: 'https://www.youtube.com/embed/RVORY42HQXM', type: 'youtube', logo: '' },
	 ...
	 MOVIES: [
         { name: 'May lamok sa loob ng kulambo', url: 'https://jungotvmedia.chanall.tv/jungotv/SolarMayLamokSaKulamboTGFULL10mbps/playlist_720p.m3u8', type: 'movies', logo: '' },
		 { name: 'Paradise Inn', url: 'https://jungotvmedia.chanall.tv/jungotv/SOLARParadiseInnTGSanitized10mbps/playlist_720p.m3u8', type: 'movies', logo: '' },
		 { name: 'Darna at Ding', url: 'https://jungotvmedia.chanall.tv/jungotv/SOLARDarnaAtDingFULL10mbps/playlist_720p.m3u8', type: 'movies', logo: '' },
		  { name: 'Captain Barbell', url: 'https://ph-en.plp4ytv25.workers.dev/nflx.mp4?file=oVxa8qQdQXaahRVfIOUCuGqSJ359NSHAVLBewkA9CSR1EFeNR5I7GquatWYlkwzK&expiry=KCSyh9bHi95Hw%2BymirQmww%3D%3D&mac=cc28900afbfc2477c90ad431b4dc98256197e548d12ef266b48a99ba832bf094', type: 'movies', logo: '' },
		 { name: 'The kingdom', url: 'https://s2.savefiles.com/hls2/01/00200/5wm1dzh8clev_n/index-v1-a1.m3u8?t=dAyUjOdyyuqoAP3cyvwepKfiB2xZr-KQrFKkKjmj9Eo&s=1760233425&e=43200&v=37991&srv=s4&i=0.0&sp=0', type: 'movies', logo: '' }
		  ],
      RADIO: [
...
function playChannel(channel) {
  ...
  // For HLS streams (M3U8 format for Movies)
  if (type === 'hls' || (/\.m3u8($|\?)/i).test(channel.url)) {
    ...
    if (Hls.isSupported()) {
      ...
      hls.on(Hls.Events.ERROR, function (event, data) {
        if (data.fatal) {
          alert('HLS stream error: ' + data.fatal);
        }
      });
  ...
  // For MP4 files (Movies)
  if (type === 'movies' || type === 'mp4') {
    const video = document.createElement('video');
    video.controls = true;
    video.src = channel.url;
    container.appendChild(video);
  ...
  // For MP3 streams (Radio with background image)
  if (type === 'mp3') {
    const audioPlayer = document.createElement('div');
    audioPlayer.className = 'radio-player';
  ...
  // For MPD streams (Shaka Player with DRM support)
  if (type === 'mpd') {
    const video = document.createElement('video');
    video.controls = true;
    container.appendChild(video);
    ...
    shakaPlayerInstance.load(channel.url).then(() => {
      ...
      alert('Error loading MPD stream with DRM: ' + error);
    });
    return;
  }

  // Handle unsupported stream types
  alert('This stream type is unsupported or invalid.');
}

// Display the list of channels for a selected category
...
function postComment() {
  ...
  if (comment) {
    ...
    item.innerHTML = `<span class="comment-username">${document.getElementById('display-username').textContent}</span> <span class="comment-time">(${currentTime})</span>: ${comment}`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Simulate a few viewers for testing
